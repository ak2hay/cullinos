...
export function GuestOpsBannersPage() {
  ...
  const { data: banners = [], isLoading } = useQuery({
    ...
    queryFn: () => guestOpsApi.listBanners(scope),
  });
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      if (editingId) return guestOpsApi.updateBanner(editingId, payload);
      return guestOpsApi.createBanner({ ...payload, scope: 'platform' });
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['guest-ops', 'banners'] });
      setForm(EMPTY);
      setEditingId(null);
      setMessage(editingId ? 'Banner updated.' : 'Platform banner created.');
      setError(null);
    },
    onError: (err: Error) => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => guestOpsApi.deleteBanner(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['guest-ops', 'banners'] });
  ...
  const deactivateMutation = useMutation({
    mutationFn: (id: string) => guestOpsApi.updateBanner(id, { isActive: false }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['guest-ops', 'banners'] });
      setMessage('Banner deactivated.');
      setError(null);
    },
    onError: (err: Error) => {
      setError(err.message);
  ...
  function startEdit(b: GuestOpsBannerRow) {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 max-w-2xl text-text-secondary">
          Platform and organization carousel slides for the Cullinos App.
        </p>
      </div>
      ...
      <section className="rounded-xl border border-white/5 bg-bg-card p-5">
        ...
        {isLoading ? (
          ...
          <p className="mt-3 text-sm text-text-muted">No banners found.</p>
        ) : (
              ...
              >
                <div className="min-w-0">
                  <p className="font-medium">
                    ...
                    <span className="ml-2 text-xs text-text-muted">{b.scope}</span>
                  </p>
                ...
                <div className="flex gap-2">
                  ...
                  {b.isActive ? (
                    <Button type="button" variant="ghost" onClick={() => deactivateMutation.mutate(b.id)}>
                      Deactivate
                    </Button>
                  ) : null}
                  <Button
