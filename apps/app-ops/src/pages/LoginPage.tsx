...
import { useAuthStore, APP_OPS_REMEMBER_KEY } from '@/stores/auth';
...
export function LoginPage() {
  ...
  const [remember, setRemember] = useState(
    () => localStorage.getItem(APP_OPS_REMEMBER_KEY) === 'true',
  );
  ...
  async function handlePasswordSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    localStorage.setItem(APP_OPS_REMEMBER_KEY, remember ? 'true' : 'false');
    try {
      ...
      <div className="w-full max-w-md rounded-2xl border border-white/10 bg-bg-secondary p-8">
        <div className="mb-8 text-center">
          ...
          <p className="mt-1 text-sm text-text-secondary">
            {challengeToken
              ...
              : `Manage the ${RKYVES_BRAND.product} guest app — users, rich push, and content.`}
          </p>
        </div>
        ...
        {challengeToken ? (
          <form onSubmit={handleOtpSubmit} className="space-y-4">
            ...
            <Button type="button" variant="ghost" disabled={loading} onClick={handleResend} className="w-full">
              Resend code
            </Button>
            <Button
              type="button"
