...
export function Button({ variant = 'primary', loading, className = '', children, disabled, ...props }) {
    return (_jsx("button", { className: `inline-flex h-11 items-center justify-center rounded-lg px-4 text-sm font-medium transition-colors ${variants[variant]} ${className}`, disabled: disabled || loading, ...props, children: loading ? 'Please wait…' : children }));
}
...
export function Select({ label, options, id, className = '', ...props }) {
    ...
    return (_jsxs("div", { className: "space-y-2", children: [_jsx("label", { htmlFor: selectId, className: "block text-sm font-medium text-text-secondary", children: label }), _jsx("select", { id: selectId, className: `h-11 w-full rounded-lg border border-white/10 bg-bg-card px-3 text-sm text-text-primary outline-none transition focus:border-brand-primary focus:ring-2 focus:ring-brand-primary/20 ${className}`, ...props, children: options.map((opt) => (_jsx("option", { value: opt.value, children: opt.label }, opt.value))) })] }));
}
