...
export function StockTransferPage() {
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    setMessage(null);
    if (!fromOutletId || !toOutletId || !inventoryItemId || !quantity) return;
    ...
    <div className="mx-auto max-w-2xl space-y-8">
      <div>
        ...
        <p className="mt-1 text-text-secondary">
          Move inventory between outlets in your network.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5 rounded-xl border border-white/5 bg-bg-card p-6">
        ...
        <label className="block">
          <span className="text-sm text-text-secondary">To outlet</span>
          <select
            required
            value={toOutletId}
        ...
        <label className="block">
          <span className="text-sm text-text-secondary">Inventory item</span>
          <select
            required
