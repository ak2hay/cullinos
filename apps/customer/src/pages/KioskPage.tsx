...
export function KioskPage() {
  ...
  const placeMutation = useMutation({
    mutationFn: async () => {
      if (!organizationId || !outletId) throw new Error('Store not loaded');
    ...
    <div className="flex min-h-screen flex-col bg-[radial-gradient(ellipse_at_top,_#1a1a2e_0%,_#0f0f1a_55%)]">
      <header className="flex items-center justify-between border-b border-white/5 px-6 py-5">
        <div>
          ...
          <h1 className="text-2xl font-bold">{outletName ?? 'Order here'}</h1>
        </div>
        <button
          type="button"
          ...
          <div className="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-4">
            {items.map((item) => (
              <button
                ...
                className="flex min-h-[140px] flex-col items-start justify-between rounded-3xl border border-white/10 bg-bg-card p-5 text-left transition active:scale-[0.97] hover:border-brand-primary/40"
              >
                <span className="line-clamp-3 text-xl font-semibold leading-tight">
                  {item.name}
                </span>
                <span className="mt-4 rounded-xl bg-brand-primary/15 px-3 py-1.5 font-mono text-lg font-bold text-brand-primary">
