...
export const tablesApi = {
  ...
  listServiceRequests: (outletId: string, status = 'open,acknowledged') => {
    const q = new URLSearchParams({ status });
    return apiRequest<ServiceRequest[]>(
      `/tables/outlets/${outletId}/service-requests?${q.toString()}`,
    );
  },

  acknowledgeServiceRequest: (outletId: string, id: string) =>
    apiRequest<ServiceRequest>(
      `/tables/outlets/${outletId}/service-requests/${id}/acknowledge`,
      { method: 'PATCH' },
    ),

  resolveServiceRequest: (outletId: string, id: string) =>
    apiRequest<ServiceRequest>(
      `/tables/outlets/${outletId}/service-requests/${id}/resolve`,
      { method: 'PATCH' },
    ),
};
...
export const ordersApi = {
  ...
  removeItem: (orderId: string, itemId: string) =>
    apiRequest<Order>(`/orders/${orderId}/items/${itemId}/remove`, {
      method: 'POST',
    }),
