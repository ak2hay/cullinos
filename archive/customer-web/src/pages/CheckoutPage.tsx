...
export function CheckoutPage() {
  ...
  const placeOrderMutation = useMutation({
    mutationFn: async () => {
      ...
      const organizationSlug = useSessionStore.getState().organizationSlug;
      const outletSlugValue = useSessionStore.getState().outletSlug;
      if (!organizationSlug || !outletSlugValue) throw new Error('Store not loaded');
    ...
    return (
      <CustomerLayout showCart={false}>
        <div className="p-8 text-center">
          <div className="mx-auto mb-2 inline-flex items-center gap-2 rounded-full border border-brand-primary/30 bg-brand-primary/10 px-3 py-1 text-xs font-medium text-brand-primary">
            Order confirmed
          </div>
          <h1 className="font-display text-2xl font-extrabold tracking-tight">You’re all set</h1>
          <p className="mt-6 text-sm uppercase tracking-widest text-text-muted">Pickup code</p>
          <p className="mt-2 font-mono text-6xl font-bold leading-none tracking-widest text-brand-primary sm:text-7xl">
            {success.pickupCode}
          </p>
          <div className="mx-auto mt-6 inline-block rounded-2xl border border-white/10 bg-bg-card p-3">
            <img
              src={qrUrl}
          ...
          <p className="mt-4 text-text-secondary">
            Show this code at the counter or pickup board.
          </p>
  ...
  return (
    <CustomerLayout showCart={false}>
      <div className="p-4">
        ...
        <h1 className="mb-2 font-display text-xl font-semibold tracking-tight">Checkout</h1>
        <ol className="mb-6 flex gap-2 text-xs text-text-muted">
          <li className="rounded-full bg-brand-primary/15 px-2.5 py-1 text-brand-primary">1 Cart</li>
          <li className="rounded-full bg-brand-primary px-2.5 py-1 text-bg-primary">2 Details</li>
          <li className="rounded-full bg-bg-card px-2.5 py-1">3 Confirm</li>
        </ol>
