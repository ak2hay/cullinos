...
For every portal/page click-path, use **[COMPLETE_FEATURE_TEST_GUIDE.md](./COMPLETE_FEATURE_TEST_GUIDE.md)**.
...
| Guest customer | **Cullinos App** | Order online (guest OK; optional login) |
| Public visitors | **Marketing site** | cullinos.com — no login |

**Your job:** Test all **190 test cases** on **production** using a **fresh test restaurant** you create on Day 1.

For click-by-click steps on every page, use **[COMPLETE_FEATURE_TEST_GUIDE.md](./COMPLETE_FEATURE_TEST_GUIDE.md)**.
...
2. `Complete_Feature_Test_Guide.pdf` — click every portal/page (URLs + commands)
3. `QA_Tester_Handbook.pdf` (this document)
4. `QA_Quick_Start.pdf` / `Manual_Test_Plan.pdf`
5. `Quick_Reference_Card.pdf`
6. `Employee_Brief.pdf`
7. `TEST_RUN_SHEET.xlsx` (190 cases)
8. `BUG_LOG.xlsx`
9. `CREDENTIALS_LOG.xlsx`
...
1. Open Cullinos Waiter (Android) — login as Waiter staff → TC-2.1-01 **Pass**
...
1. Open `https://guest.cullinos.com/{orgSlug}/{outletSlug}` → TC-2.3-01 **Pass**
...
### 6.4 Displays hub & Order Display / CDS (TC-2.4-xx)

1. Admin → **Displays** (`/displays`) — page loads → TC-2.4-01 **Pass**
2. Copy CDS / Pickup / KDS / Promo URLs and open in new tabs → TC-2.4-02 **Pass** or **Blocked**
3. Place a takeaway/POS order; confirm it appears in **Preparing** → TC-2.4-03 **Pass** or **N/A**
4. After kitchen marks ready, confirm **Ready** column → TC-2.4-04 **Pass** or **N/A**
5. Legacy bookmarks `/cds`, `/kds`, `/promo-display`, `/pickup-queue` redirect to `/displays` → TC-2.4-05 **Pass**
...
### 6.7 Cullinos App Guest screens (TC-2.7-xx)

Follow [COMPLETE_FEATURE_TEST_GUIDE.md](./COMPLETE_FEATURE_TEST_GUIDE.md) chapter 10 for Home, Explore, Scan, Offers, Profile, Orders, Wallets, Kiosk.
...
| Customers | `/customers` | Create / search; stamps / redeem |
| Coupons | `/coupons` | Create coupon |
| Loyalty | `/loyalty` | Save settings; tiers / rewards |
| Recipes | `/recipes` | Create recipe; appears in list |
| Delivery | `/delivery` | Add zone / list loads |
| Payments | `/payments` | Razorpay / Cashfree Save (N/A without keys) |
| Reservations | `/reservations` | Create reservation; copy book link |
| Purchasing / Suppliers | `/purchasing`, `/suppliers` | Draft PO; add supplier |
| Aggregators | `/aggregators` | Swiggy/Zomato tabs |
...
| Displays | `/displays` | Copy/open KDS, CDS, Pickup, Promo URLs (legacy `/kds` `/cds` redirect here) |
| Digital Ordering | `/kiosk` | Storefront / kiosk URL shown |
...
| Reports | `/reports` | Revenue, top items, Export |
| Settings | `/settings` | Form Save settings; add location / tax / device |
| Login | `/login` | Logout and re-login works |

Mark TC-3-01 through TC-3-45 in Excel. Full click-paths: [COMPLETE_FEATURE_TEST_GUIDE.md](./COMPLETE_FEATURE_TEST_GUIDE.md).

**Note:** Promo Email is **Super Admin only** (`/promo-email`), not Admin.
...
**Background:** Use Admin **Customers** (`/customers`) to create/search customers, and **Loyalty** (`/loyalty`) / **Coupons** (`/coupons`) for program setup. Stamp add may still be done via API (Swagger) if needed for reward thresholds.

**Pre-seeded data (demo org only — not your fresh QA tenant):**
- Loyalty tier: Stamp Card — 10 stamps = 100 points = free drink reward
- Coupon: `WELCOME10` — 10% off, minimum order ₹200

On a fresh QA tenant, create tiers/rewards/coupons in Admin UI first (see Complete Feature Test Guide).

**Stamp test steps:**
1. Place an order via the Cullinos App so a customer record exists.
...
5. `POST /api/v1/loyalty/customers/{customerId}/stamp` — execute 10 times (or use Admin **Add stamp** if shown).
6. After 10 stamps the tier reward triggers automatically (100 loyalty points, stamp count resets).
7. Admin Customers / Loyalty shows updated state → **Pass**.

**Coupon test steps:**
1. Add items to cart in Cullinos App totalling ≥ ₹200.
2. At checkout enter a coupon you created (or `WELCOME10` on demo org).
3. Verify discount is applied → **Pass**.
...
5. **Promo Email** (`/promo-email`) — platform only → TC-5-16, TC-5-29 **Pass**
...
8. **Cullinos App / Guest Ops** — overview + marketplace/discover/banners/push/offers/reviews/users/analytics/runtime → TC-5-21 through TC-5-28 **Pass**
9. Tenant detail **Open as tenant** → TC-5-30 **Pass**
10. Logged out: `/forgot-password` → TC-5-20 **Pass**
11. **Destructive (QA tenant only):**
...
- [ ] All 190 test cases marked Pass / Fail / Blocked / N/A in TEST_RUN_SHEET.xlsx
